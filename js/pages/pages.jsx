...
function DirectoryPage({ navigate, lang }) {
  ...
  const features = en ? en.features : [
    {
      ...
      dParagraphs: [
        ...
        [
          { s: 'Проект начал работу 1 сентября 2025 года. Наша цель — объединить в одном ресурсе проверенную, структурированную и практически применимую медицинскую информацию. Базовой единицей данных является официальная инструкция по медицинскому применению и общая характеристика лекарственного средства — документы, на которые специалисты опираются при назначении, отпуске, консультировании и оценке терапии.' },
        ],
        [
          { s: 'В Справочнике доступны официальные инструкции, примеры оформления медицинских рецептов, материалы по фармакологии, клинические протоколы, статьи, калькуляторы и медицинские шкалы. Отдельное направление развития — интеллектуальные инструменты. Мы внедряем сервис «ИИ — фармаколог», который помогает оценивать лекарственные взаимодействия с опорой на международные базы данных и делает работу с лекарственной информацией быстрее и удобнее.' },
        ],
      ],
    },
    {
      ...
      dParagraphs: [
        [
          { s: 'Мы открыты к сотрудничеству с врачами, которые хотят делиться профессиональным опытом, развивать медицинское образование и создавать полезные цифровые инструменты для коллег.' },
        ],
        [
          { s: 'Если у вас есть идеи для публикаций, авторские материалы, клинические разборы, мастер-классы, образовательные модули или практические рекомендации, мы готовы обсудить их размещение на платформе. Нам особенно интересны материалы, которые помогают врачу быстрее ориентироваться в клинической ситуации, принимать обоснованные решения и использовать лекарственные средства рационально.' },
        ],
        [
          { s: 'Отдельно приглашаем к сотрудничеству авторов медицинских шкал, клинических алгоритмов, расчетных моделей и разработок, созданных в рамках научной, диссертационной или практической работы. Наши ИТ-специалисты могут помочь превратить такие материалы в удобные цифровые калькуляторы или интерактивные инструменты с указанием авторства и профессионального вклада разработчиков.' },
        ],
        [
          { s: 'Если вы видите, какие разделы стоит добавить, какие калькуляторы внедрить, какие клинические темы требуют более точного освещения, — напишите нам. Мы хотим, чтобы Справочник ЛС ФармКонсилиум развивался вместе с медицинским сообществом и отражал реальные потребности практикующих врачей.' },
        ],
      ],
    },
    {
      ...
      dParagraphs: [
        ...
        [
          { s: 'Уже сейчас большое число провизоров и фармацевтов используют Справочник в своей работе. Но сделать его по-настоящему удобным, точным и прикладным можно только вместе с профессиональным сообществом. Мы приглашаем вас делиться предложениями по структуре карточек лекарственных средств, наполнению разделов, формату рецептов, справочных материалов и дополнительных инструментов.' },
        ],
        [
          { s: 'Нам интересны темы, которые действительно важны для аптечной практики: консультирование пациентов, частые вопросы в аптеке, замена препаратов, особенности применения безрецептурных средств, лекарственные взаимодействия, отпуск по рецепту, хранение, безопасность и рациональное использование лекарств.' },
        ],
        [
          { s: 'Если у вас есть материалы, памятки, профессиональные комментарии, идеи для статей или предложения по улучшению Справочника — мы готовы к диалогу. Наша задача — создать ресурс, который помогает провизору и фармацевту быстро находить точную информацию и уверенно применять ее в ежедневной работе.' },
        ],
      ],
    },
    {
      ...
      dParagraphs: [
        [
          { s: 'Для производителей лекарственных средств Справочник ЛС ФармКонсилиум — это профессиональная цифровая площадка, где информация о препарате представлена в формате, удобном для врачей, провизоров и фармацевтов. Здесь важны не рекламные обещания, а точные данные, корректная структура и практическая применимость информации в ежедневной медицинской и аптечной работе.' },
        ],
        [
          { s: 'Мы можем разместить и структурировать официальные инструкции, общие характеристики лекарственных средств, примеры оформления медицинских рецептов, базовую фармакологическую информацию, данные по формам выпуска, дозировкам, показаниям, противопоказаниям, особенностям применения и другим важным разделам. Такой формат помогает специалисту быстрее находить нужную информацию и снижает риск ошибочной интерпретации данных.' },
        ],
        [
          { s: 'Для компаний, работающих на нескольких рынках СНГ, особенно ценен мультирыночный формат: один препарат может иметь несколько версий инструкций для разных стран. Это позволяет аккуратно представить портфель, учитывать локальные регуляторные различия и поддерживать актуальность информации по каждому рынку.' },
        ],
        [
          { s: 'Контент проходит редакционную проверку: тексты анализируют практикующие врачи-редакторы и клинический фармаколог. На карточках может фиксироваться дата обновления, что повышает прозрачность и доверие к информации. Мы открыты к сотрудничеству с производителями, которые заинтересованы в точном, профессиональном и ответственном представлении своих лекарственных препаратов для медицинского сообщества.' },
        ],
      ],
    },
  ];
  ...
  const stats = en ? en.stats : [
    ...
    { n: '03', t: 'ОХЛС разных стран СНГ', d: 'Только официальные инструкции по медицинскому применению' },
